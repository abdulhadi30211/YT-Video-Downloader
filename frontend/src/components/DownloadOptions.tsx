...
import { Loader2, Download, FileVideo, FileAudio, Music, Volume2, VolumeX, Play, Clock } from 'lucide-react';
...
const DownloadOptions: React.FC<DownloadOptionsProps> = ({ videoFormats, videoTitle, videoUrl }) => {
  const [selectedFormatId, setSelectedFormatId] = useState<string | null>(null);
  const [downloadType, setDownloadType] = useState<'video' | 'audio'>('video'); // Changed to only video/audio
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      console.log('Frontend: Initiating download request with:', { videoUrl, selectedFormatId, filename });
      ...
      const downloadUrl = `/api/download?url=${encodeURIComponent(videoUrl)}&formatId=${encodeURIComponent(selectedFormatId!)}&filename=${encodeURIComponent(filename)}&socketId=${encodeURIComponent(downloadSocketId!)}`;
  ...
  const handleAd = () => {
    withSmartlink(() => {
      console.log('Ad window opened for download');
    });
  };
    ...
    <div className="w-full max-w-md space-y-6 p-6 border rounded-xl shadow-lg bg-card text-card-foreground">
      ...
      >
        ...
        <div className={`flex items-center space-x-2 p-3 border rounded-lg hover:bg-accent cursor-pointer transition-colors ${downloadType === 'audio' ? 'border-primary bg-accent' : ''}`}>
          ...
          <Label htmlFor="audio-type" className="flex items-center gap-2">
